    ...
    return this.lock(async ()=>{
      ...
      const new_list = await Promise.all(this.list.map(async (n, index) => {
        try {
          ...
          console.log("UPDATE NODE : ", updated_node, n);
          if(JSON.stringify(updated_node) != JSON.stringify(n)){
            changes ++;
            setImmediate(()=>this.emit("update", updated_node));
          }
          return updated_node;
        }catch(e){
      ...
      if(0 < changes){
        setImmediate( () =>this.emit("change", this.list));
      }
      return new_list;
    }).then((new_list)=>{
