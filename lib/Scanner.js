...
class Scanner extends EventEmitter{
  constructor(opts={}){
    ...
    this._data = initial;
    const browser = mdns.createBrowser(mdns.tcp(service), Object.assign(
  ...
  refresh(timeout){
    return this.lock(async ()=>{
      const orig_string = JSON.stringify(this.list); //Make a full copy of original list
      //this.list being an array of object, they update themselves
      await Promise.all(this.list.map(async n => {
        try {
          return await n.withStatus();
        }catch(e){
          this.emit("error",new BrowseError(`Failed to fetch node status for ${n.name} : ${e.message}`));
        }
      }));
      if(orig_string != JSON.stringify(this.list)){
        //list changed.
  ...
  find(item){
    const idx = this._data.findIndex(n => n.isSame(item));
